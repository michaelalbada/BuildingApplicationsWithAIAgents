...
def load_and_prepare_dataset(ds_name: str, tokenizer, max_train: int, max_eval: int) -> DatasetDict:
    ...
    test_ds = split["test"]
    # Safely select up to available examples
    train_sel = min(max_train, len(train_ds))
    test_sel = min(max_eval, len(test_ds))
    split["train"] = train_ds.select(range(train_sel))
    split["test"] = test_ds.select(range(test_sel))
    return split
...
def main():
    ...
    # --- Optional hub push --------------------------------------------------
    if args.push_to_hub:
        maybe_push_to_hub(trainer, tokenizer, args.hf_username, args.output_dir)
